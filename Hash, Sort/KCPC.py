...
for _ in range(test):
  t, q, m, l = map(int, sys.stdin.readline().split())
  # 딕셔너리 3개 생성
  # 문제에 대한 점수, 제출 횟수, 제출 시간.
  score = [{} for _ in range(t)]
  tick = {}
  time = {}
  for i in range(l):
    id, qs, sc = map(int, sys.stdin.readline().split())
    # 점수
    if qs not in score[id-1] or qs in score[id-1].keys() and score[id-1][qs] < sc:
      score[id-1][qs] = sc
    
    # 횟수
    if id not in tick.keys():
      tick[id] = 1
    else:
      tick[id] += 1
    # 시간
  ...
  sor = []
  for i in range(t):
    sor.append([sum(score[i].values()), tick[i+1], time[i+1], i+1])

  sor = sorted(sor, key = lambda sor : sor[2])
  sor = sorted(sor, key = lambda sor : sor[1])
  sor = sorted(sor, key = lambda sor : sor[0], reverse= True)
