...
# 거리가 제일 긴 곳 확인해서 거기만 커버할 수 있으면 됨.
# 마지막에 양 끝 길이만 비교해주면 됨.

for i in range(1, len(spot_list)):
  if max_len < spot_list[i] - spot_list[i-1]:
    max_len = spot_list[i] - spot_list[i-1]

print(max(spot_list[0], path_length - spot_list[-1], math.ceil(max_len/2)))